...
//----------------------------Step 1----------------------------//
// Key Points.
// We are given a integer , we return true if it is a power of Four if not return false.
//----------------------------Step 2----------------------------//
// Our input : integer.
...
//As a passionate mathematician we can simply use log_2(n) and if the result is an integer then it is a power of Four (we return true). 
//----------------------------Step 8----------------------------//
// Code Walkthrough:
...
var LOG = function LOG(val, expo) {
  return Math.log2(val) / Math.log2(expo);
};

var isPowerOfFour = function isPowerOfFour(n) {
  if (n <= 0) return false;
  return LOG(n, 4) % 1 === 0;
};
